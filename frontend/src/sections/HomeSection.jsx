                                ...
                                children: [
                                  c.jsxs('div', {
                                    ...
                                    children: [
                                      c.jsx('p', {
                                        className:
                                          'min-w-0 flex-1 truncate text-xs font-semibold text-gray-900 dark:text-gray-100',
            ...
            c.jsx('div', {
              ...
              children: filteredHomeClientsInMission.length === 0
                ...
                : filteredHomeClientsInMission.map((client) => {
                    const missionProducts = homeClientMissionProductsMap[client.id] || [];
                    const annotatedCount = missionProducts.filter(
                      (product) => String((product && product.status) || '').toUpperCase() === 'ANNOTATED',
                    ).length;
                    const inReviewCount = missionProducts.filter(
                      (product) => String((product && product.status) || '').toUpperCase() === 'IN_REVIEW',
                    ).length;
                    const rejectedCount = missionProducts.filter(
                          ...
                          children: [
                            c.jsx('button', {
                              ...
                              onClick: (event) => {
                                ...
                                if (copiedClientShareLinks.includes(key)) {
                                  setCopiedClientShareLinks((values) =>
                                    values.filter((value) => value !== key),
                                  );
                                  return;
                                }
                                copyClientMissionShareLink(null, client);
                              },
                              className: `w-8 h-8 rounded-md border flex items-center justify-center transition ${
                                copiedClientShareLinks.includes(`client-history-${client.id}`)
                            ...
                            c.jsx('button', {
                              ...
                              onClick: (event) => {
                                event.stopPropagation();
                                typeof openClientPaymentModal === 'function' &&
                                  openClientPaymentModal(client);
                              },
                              className:
                            ...
                            c.jsx('button', {
                              ...
                              onClick: (event) => {
                                event.stopPropagation();
                                copyAnnotatedMissionBreakdown(activeMission, client);
                              },
                              title: 'Enviar desglose por WhatsApp',
                              className: `w-8 h-8 rounded-md border flex items-center justify-center transition ${
                                copiedMissionClients.includes(`home-${activeMission.id}-${client.id}`)
                                  ? 'border-sky-300 bg-sky-100 text-sky-700 dark:border-sky-700 dark:bg-sky-950/35 dark:text-sky-200'
                              ...
                              children: c.jsx('span', {
                                ...
                                children: copiedMissionClients.includes(
                                  `home-${activeMission.id}-${client.id}`,
                                )
                                  ? 'done'
                                  : 'receipt_long',
                              }),
